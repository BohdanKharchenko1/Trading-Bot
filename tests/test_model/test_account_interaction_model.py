...
class TestAccountInteractionModel(unittest.TestCase):
    ...
    def test_initialization(self, mock_http):
        # Test initialization and HTTP session creation
        AccountInteractionModel(self.mock_config_model)
        mock_http.assert_called_with(testnet=True, api_key="test_key", api_secret="test_secret")

    @patch('pybit.unified_trading.HTTP.get_wallet_balance')
    def test_get_usdt_balance(self, mock_get_wallet_balance):
        # Setup mock response
        mock_get_wallet_balance.return_value = {
            'retCode': 0,
            'result': {
                'list': [
                    {'coin': 'USDT', 'availableToWithdraw': '500'}
                ]
            }
        }
        balance = self.account_model.get_usdt_balance()
